...
// ==========================================
// 1. HANDLE SOFT DELETE (Deactivate Staff)
// ==========================================
...
// ==========================================
// 2. HANDLE ADD STAFF FORM SUBMISSION
// ==========================================
...
// Hide purely inactive staff, but show Active and Suspended
...
if ($sort == 'name_desc') {
    ...
    $order_clause = "ORDER BY FIELD(staff_role, 'Admin', 'Manager', 'Staff'), staff_name ASC";
} else {
    $order_clause = "ORDER BY staff_name ASC"; // Default A-Z
}
    ...
    <div style="background: white; padding: 24px; border-radius: 12px; border: 1px solid #e5e7eb; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
        ...
        <form action="manage_admin.php" method="POST">
            <div style="margin-bottom: 16px;">
                ...
                <input type="text" name="staff_name" required placeholder="e.g. Ali bin Abu" 
            ...
            <div style="margin-bottom: 16px;">
                ...
                <input type="email" name="staff_email" required placeholder="staff@musicstore.com" 
                       style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px; outline: none; box-sizing: border-box;">
            </div>

            <div style="margin-bottom: 16px;">
                <label style="display: block; font-size: 0.85rem; font-weight: 600; margin-bottom: 6px; color: #4b5563;">Phone Number</label>
                <input type="text" name="staff_phone_number" placeholder="e.g. 012-3456789" 
                       style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px; outline: none; box-sizing: border-box;">
            </div>

            <div style="margin-bottom: 16px;">
                <label style="display: block; font-size: 0.85rem; font-weight: 600; margin-bottom: 6px; color: #4b5563;">Home Address</label>
                <textarea name="staff_address" rows="3" placeholder="Full mailing address..." 
                          style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px; font-family: inherit; outline: none; box-sizing: border-box; resize: vertical;"></textarea>
            </div>

            <div style="margin-bottom: 16px;">
                <label style="display: block; font-size: 0.85rem; font-weight: 600; margin-bottom: 6px; color: #4b5563;">Temporary Password *</label>
                <input type="password" name="staff_password" required placeholder="••••••••" 
                       style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px; outline: none; box-sizing: border-box;">
            </div>
                ...
                <tbody>
                    <?php if (mysqli_num_rows($staff_result) > 0): ?>
                        ...
                        <tr style="border-bottom: 1px solid #f3f4f6;">
                            <td style="padding: 16px 8px; color: #6b7280; font-size: 0.9rem;">
                            ...
                            <td style="padding: 16px 8px;">
                                <div style="display: flex; flex-direction: column; gap: 8px; align-items: flex-start;">
                                    <span style="background: <?php echo ($row['staff_role'] == 'Admin') ? '#ede9fe' : (($row['staff_role'] == 'Manager') ? '#e0e7ff' : '#f3f4f6'); ?>; 
                                                 color: <?php echo ($row['staff_role'] == 'Admin') ? '#5b21b6' : (($row['staff_role'] == 'Manager') ? '#3730a3' : '#4b5563'); ?>; 
                                                 padding: 4px 10px; border-radius: 12px; font-size: 0.75rem; font-weight: 600;">
                                        <?php echo htmlspecialchars($row['staff_role']); ?>
                                    </span>
                                    
                                    <span style="background: <?php echo ($row['status'] == 'Active') ? '#d1fae5' : '#fef3c7'; ?>; 
                                                 color: <?php echo ($row['status'] == 'Active') ? '#065f46' : '#92400e'; ?>; 
                                                 padding: 4px 10px; border-radius: 12px; font-size: 0.75rem; font-weight: 600;">
                            ...
                            <td style="padding: 16px 8px; text-align: right;">
                                <?php if ($row['staff_id'] != $_SESSION['staff_id']): ?>
                                    <a href="manage_admin.php?delete_id=<?php echo $row['staff_id']; ?>" 
                                       ...
                                       onclick="return confirm('Are you sure you want to revoke access for <?php echo addslashes($row['staff_name']); ?>?')">
                                       Revoke Access
                                    </a>
                                <?php else: ?>
                                    <span style="font-size: 0.85rem; color: #9ca3af; font-weight: 500;">Current User</span>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="4" style="text-align: center; padding: 40px; color: #9ca3af; font-size: 0.95rem;">
