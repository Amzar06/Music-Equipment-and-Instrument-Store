...
// ==========================================
// 1. HANDLE SOFT DELETE ACTION
// ==========================================
if (isset($_GET['delete_id'])) {
    $delete_id = intval($_GET['delete_id']);
    
    // FYP REQUIREMENT: SOFT DELETE. We do NOT use "DELETE FROM" here.
    // We update the status so old sales/rentals don't break.
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_product'])) {
    ...
    if (isset($_POST['for_sale'])) {
        $prod_qty = intval(preg_replace('/[^0-9]/', '', $_POST['prod_sale_qty'] ?? 0));
    } elseif (isset($_POST['for_rent'])) {
        $prod_qty = intval(preg_replace('/[^0-9]/', '', $_POST['prod_rental_qty'] ?? 0));
    }
...
// ==========================================
// 3. FETCH DATA WITH SEARCH AND SORT LOGIC
// ==========================================
...
// Base condition: NEVER show soft-deleted items in the live inventory
...
<div style="display: grid; grid-template-columns: 1fr 2fr; gap: 32px; align-items: start;">
    ...
    <div class="table-container" style="margin-top: 0;">
        <h3 style="margin-bottom: 16px; font-weight: 700; color: var(--text-main);">Current Stock</h3>
        
        <div style="background: #f9fafb; border: 1px solid #e5e7eb; padding: 16px; border-radius: 8px; margin-bottom: 20px;">
            <form action="admin_products.php" method="GET" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center;">
                <input type="text" name="search" placeholder="Search product or category..." value="<?php echo htmlspecialchars($search); ?>" 
                       style="flex-grow: 1; padding: 10px; border: 1px solid #d1d5db; border-radius: 6px; outline: none;">
                
                <select name="sort" style="padding: 10px; border: 1px solid #d1d5db; border-radius: 6px; background: white; outline: none;">
                ...
                <button type="submit" style="padding: 10px 20px; background: #374151; color: white; border: none; border-radius: 6px; font-weight: 600; cursor: pointer;">
                    Apply
                </button>
                <?php if(!empty($search) || $sort != 'newest'): ?>
                    <a href="admin_products.php" style="color: #ef4444; text-decoration: none; font-size: 0.9rem; font-weight: 600; padding: 10px;">Clear</a>
                <?php endif; ?>
            </form>
        </div>
                        ...
                        <tr style="border-bottom: 1px solid #f3f4f6;">
                            ...
                            <td style="padding: 12px 8px;">
                                <div style="font-size: 0.85rem; display: flex; flex-direction: column; gap: 4px;">
                                    <?php if($row['prod_sale_price'] > 0): ?>
                                        <span style="background: <?php echo ($row['prod_qty'] > 0) ? '#d1fae5' : '#fee2e2'; ?>; color: <?php echo ($row['prod_qty'] > 0) ? '#065f46' : '#991b1b'; ?>; padding: 2px 8px; border-radius: 12px; display: inline-block; width: fit-content;">
                                            Sale Pool: <?php echo $row['prod_qty']; ?>
                                        </span>
                                    <?php endif; ?>
                                    
                                    <?php if($row['prod_rental_price'] > 0): ?>
                                        <span style="background: <?php echo ($row['prod_qty'] > 0) ? '#dbeafe' : '#fee2e2'; ?>; color: <?php echo ($row['prod_qty'] > 0) ? '#1e40af' : '#991b1b'; ?>; padding: 2px 8px; border-radius: 12px; display: inline-block; width: fit-content;">
                                            Rent Pool: <?php echo $row['prod_qty']; ?>
                                        </span>
                                    <?php endif; ?>
                                </div>
                            </td>
